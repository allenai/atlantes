...
EXPERIMENTAL_CONFIG_TRAIN = get_experimental_config_buoy()
EXPERIMENTAL_CONFIG_MODEL = get_experimental_config_buoy("hyperparameters")
DATA_CONFIG = get_experimental_config_buoy("data")


def init_and_launch_entity_trainer(
    rank: int,
    world_size: int,
    data_config: dict,
    ...
    trainer = AtlasEntityTrainer(
        ...
        label_enum=AtlasEntityLabelsTraining,
    )
    # Launch training job
...
def launch_train_atlas_entity(
    ...
    """
    launch_ddp_job(
        init_and_launch_entity_trainer,
        data_config,
